...
corrplot::corrplot(cor(breast_cancer[2:31]), type = "full",
                   tl.cex = .7, tl.col = "black", title = "Fig 1 Correlation Plot")
```
...
In the part of Pathwise logistic-lasso model, we firstly select the maximum $\lambda$ which is the inner product of response and all variables and $\lambda_{max}$ = 218.13. We show the lasso solution path **(Fig 3)**. By using a sequence of $\lambda$ to minimize the objective function and have a relatively higher AUC **(Fig 4)**, the minimum value would be taken under when $\lambda$ = 1.57 which means the optimal solution will be computed for this optimal $\lambda$ value.
...
ggplot(data = tidy.path, aes(x = loglambda, y = value, color = beta, group = beta)) +
  ...
  scale_x_reverse() + labs(title = "Fig 3 Lasso Solution Path")
```
...
result = read.csv("cv_results.csv") %>% 
    ...
    theme(plot.title = element_text(hjust = 0.5)) + ylim(0.97, 1.002)
result
```
