...
### Try cd_lasso...
```{r}
a <- cd_lasso(bc_scale2_x, bc_scale2_y, betavec = rep(1, 31), lambda = 1)
```
...
pathwise_cd_lasso <- function(dat, y, lambda) {
  ...
  for (j in 1:length(lambda)) {
    ...
    # warm start
    betavec <- cd_results[nrow(cd_results), -c(1, 2)]
...
# pathwise cd_lasso
res <- pathwise_cd_lasso(bc_scale2_x, bc_scale2_y, seq(5, -3, length = 10))

as.tibble(res) %>% dplyr::select(-c(1,3)) %>%
  pivot_longer(4:dim(res)[2] - 2, names_to = "beta", values_to = "value") %>% 
  ggplot(aes(x = log(lambda), y = value, color = beta)) + 
  geom_line(show.legend = FALSE) + scale_x_reverse(limits = c(5, -3)) +
  ylim(c(-20, 20))
```
