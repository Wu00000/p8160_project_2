...
## NR results examples
```{r}
View(NewtonRaphson(bc_scale1_x, compute_stat, rep(1, 11)))
...
max_lambda1 <- max(t(bc_scale1_x) %*% bc_scale1_y)
max_lambda2 <- max(t(bc_scale2_x) %*% bc_scale2_y)

a <- cd_lasso(bc_scale1_x, y, rep(1,11), lambda = 1)
View(a)
```
...
### Try...
```{r}
b <- pathwise_cd_lasso(bc_scale1_x, bc_scale1_y, rep(1, 11), lambda = seq(5, 0, length = 100))
View(b)
```
...
cv = function(raw_data, lambda_vec, kfolds = 5, betavec){
  ...
  for (i in 1:length(lambda_vec)){
    ...
    for (j in 1:kfolds){
      ...
      res = pathwise_cd_lasso(train_x, train_y, betavec, lambda_vec[i])
      curbetavec = res[dim(res)[1], -c(1, 2)]
