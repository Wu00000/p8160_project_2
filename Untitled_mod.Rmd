...
### Try...
```{r}
a <- cd_lasso(bc_scale1_x, rep(1,11), lambda = 2)
```
...
pathwise_cd_lasso <- function(dat, betavec, lambda, maxier = 2000, tol = 1e-10) {
  ...
  for (i in 1:length(lambda)) {
    cd_results <- cd_lasso(dat, betavec, lambda[i], maxier, tol)
    
    pathwise_res <- rbind(pathwise_res, 
                          c(i, lambda[i], as.tibble(cd_results)[, -c(1, 2)]))
  }
...
### Try...
```{r}
pathwise_cd_lasso(bc_scale1, rep(1, 11), lambda = exp(seq(5, -5, length = 100)))
```
