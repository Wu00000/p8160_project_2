...
## NR results (10 means & all 30 predictors)
```{r}
...
cv = function(raw_data, lambda_vec, kfolds = 5, betavec){
  ...
  final_res <- c(fold = 0, lambda = 0, beta = betavec, auc = 0)
...
set.seed(2022)
cv_res2 <- cv(bc_scale2, lambda_vec = exp(seq(log(max_lambda2), 0, length = 50)), betavec = rep(1, 31))
```

### 
```{r}
# cd_lasso using best lambda
as.tibble(cv_res2) %>% 
  group_by(fold) %>% 
  arrange(desc(auc)) %>% 
  do(head(., n = 5))
...
### Lasso Solution Path
```{r}
as.tibble(lambda_auc) %>% filter(fold != 0) %>% 
  pivot_longer(4:dim(lambda_auc)[2] - 1, names_to = "beta", values_to = "value") %>% 
  ggplot(aes(x = log(lambda), y = value, color = beta)) + 
  geom_line(show.legend = FALSE) + scale_x_reverse(limits = c(log(max_lambda1), 0)) + 
  facet_wrap(~ fold) + theme_minimal()
```
