...
cd_lasso <- function(dat, betavec, lambda, maxier = 2000, tol = 1e-10) {
  x <- dat # n * (p + 1) matrix, standardized
  y <- breast_cancer$diagnosis # n * 1 vector
  i <- 0
  loglik <- 0
  prevloglik <- -Inf # ensure iterations
...
### Try...
```{r}
a <- cd_lasso(bc_scale1_x, rep(1,11), lambda = 10)
```
...
pathwise_cd_lasso <- function(dat, betavec, lambda, maxier = 2000, tol = 1e-10) {
  ...
  for (j in 1:length(lambda)) {
    cd_results <- cd_lasso(dat, betavec, lambda[j], maxier, tol)
...
cv = function(raw_data, lambda_vec, kfolds = 5, betavec){
  ...
  for (i in length(lambda_vec)){
    ...
    for (j in 1:kfolds){
      ...
      print(validation)
      training = raw_data[fold_index != j, -ncol(raw_data)]  #454
      cd_results = cd_lasso(training, betavec, lambda[i])
      curbetavec = cd_results[dim(cd_results)[1], -c(1, 2)]
      u = validation[,-ncol(validation)] %*% curbetavec
      prob = exp(u)/(1 + exp(u))
      fold_auc[j] = auc(prob, validation$outcome)
    }
    # auc fill in
    auc[i] = mean(fold_auc)
